...
[[nodiscard]] constexpr T Clamp(T v, L lo, H hi) {
  if constexpr (!internal::is_convertible_without_narrowing<L, T>) {
    static_assert(std::is_integral_v<L>, "Integer Clamp's lower bound must be an integer.");
    MAYS_CHECK(std::cmp_less_equal(lo, std::numeric_limits<T>::max()));
  }
  if constexpr (!internal::is_convertible_without_narrowing<H, T>) {
    static_assert(std::is_integral_v<H>, "Integer Clamp's upper bound must be an integer.");
    MAYS_CHECK(std::cmp_greater_equal(hi, std::numeric_limits<T>::min()));
  }
  MAYS_CHECK(std::cmp_less_equal(lo, hi));
  const T lower_bounded = std::cmp_greater_equal(v, lo) ? v : T(lo);
  const T both_bounded = std::cmp_less_equal(lower_bounded, hi) ? lower_bounded : T(hi);
  return both_bounded;
}

template <typename T, typename = std::enable_if_t<!std::is_integral_v<T>>>
[[nodiscard]] constexpr T Clamp(T v, std::type_identity_t<T> lo, std::type_identity_t<T> hi) {
